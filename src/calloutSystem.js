...
function onARAnchorHover(event) {
  ...
  if (intersects.length > 0) {
    ...
    if (geometry.index && faceIndex != null) {
      ...
      const COPLANAR_THRESHOLD = 0.90; // dot product threshold (~25 degrees)
      const MAX_REGION_FACES = 80; // cap for performance
      const totalFaces = indexAttr.count / 3;

      // Build vertex → face adjacency map (once per hover — fine for interactive rates)
      ...
      while (queue.length > 0 && regionFaces.length < MAX_REGION_FACES) {
        ...
        // Find adjacent faces via shared vertices
        for (let c = 0; c < 3; c++) {
          ...
          for (const nf of neighbors) {
            ...
            // Check if neighbor face is coplanar (similar normal)
      ...
      const edgeVerts = []; // for wireframe edges
